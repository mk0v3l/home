...
main() {
	...
	*)
	...
	colstat='#bd93f9'
	if $show_powerline; then
		tmux set-option -g status-left "#[bg=${green},fg=${dark_gray}]#{?client_prefix,#[bg=$colstat],} ${left_icon} #[fg=${green},bg=${gray}]#{?client_prefix,#[fg=$colstat],}${left_sep}"
		powerbg=${gray}
	else
	...
	for plugin in "${plugins[@]}"; do
		...
		if case $plugin in custom:*) true ;; *) false ;; esac then
			...
			# IFS=' ' read -r -a colors <<< $(~/.tmux/batOk.sh)
		...
		if $show_powerline; then
			if $show_empty_plugins; then
				# tmux set-option -ga status-right "#[fg=${!colors[0]},bg=${powerbg},nobold,nounderscore,noitalics]${right_sep}i#[fg=${!colors[1]},bg=${!colors[0]}] $script "
